...
# Delete an inner collection of parent/babysitter, by parent's id / babysitter id
@app.route('/delete_inner_item_collection/<user_id>/<item_id>/<inner_collection_name>', methods=['DELETE'])
def delete_inner_item_collection(user_id, item_id, inner_collection_name):
    try:
        collection_name = request.headers.get('Collection-Name')
        doc_ref = db.collection(collection_name).document(user_id)
        inner_collection = doc_ref.collection(inner_collection_name)
